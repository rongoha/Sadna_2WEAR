...
if(isset($_POST['submit'])){
    ...
    if(empty($price) || empty($title) || empty($color) || empty($size)  || empty($picture)){
        echo "Please enter all the requested fields!";
    }else{
        $sql="INSERT INTO sales_items(price, title, color, size, condition_item, category, picture, description, user_id, gender) VALUES ('$price', '$title', '$color', '$size', '$condition_item', '$category', '$picture', '$description', '$userid', '$gender')";
        if($conn->query($sql)===TRUE){
            ...
            foreach($_FILES["pictures"]["tmp_name"] as $key=>$tmp_name) {
                ...
                if($extension=='png' || $extension=='jpg' || $extension=='jpeg' || $extension=='gif'){
                    $picture = uniqid(time()).".$extension";
                    $sql="INSERT INTO item_pictures (picture, item_id) VALUES ('$picture', '$last_id')";
                    $conn->query($sql);
                    move_uploaded_file($file_tmp, "img/$picture");
                }
            }
            
            $msg = '<div class="alert alert-success text-center">Thank you, We got your details, the item have been added to our shop :)</div>';
        }
    }
}
...
<head>
    <title> 2WEAR Shop - Sell My Clothes </title>
    <meta charset="utf-8">
            ...
            <form method="post" action="" class="col-md-9 m-auto" role="form1" name="form1" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="form-group col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control mt-1" id="title" name="title" placeholder="Title" minlength="2" maxlength="50" required>
                    </div>
                    <div class="form-group col-md-6 mb-3">
                        <label for="inputPrice">Price*</label>
                        <input type="number" min="1" max="10000000000" class="form-control mt-1" id="price" name="price" placeholder="Price" required>
                    </div>
                    ...
                    <div class="mb-3">
                        <div>
                            ...
                            <div class="form-check">
                                <input type="radio" class="form-check-input" id="dress" name="category" value="Dress">Dress
                                <label class="form-check-label" for="dress"></label>
                            </div>
                            <div class="form-check">
                                <input type="radio" class="form-check-input" id="suit" name="category" value="Suit">Suit
                                <label class="form-check-label" for="suit"></label>
                            </div>
                        </div>
                    </div>
                    <div class="mb-3 ">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select name="condition_item" required>
                ...
                <div>
                    <p>
                        <label><b> Main picture*</b>
                            <input accept="image/png, image/gif, image/jpeg, image/jpg" type="file" name="picture" id="file" required></label>
                    </p>
                </div>
                <div>
                    <p>
                        <label><b> Other pictures (Maximum 3)*</b>
                            <input accept="image/png, image/gif, image/jpeg, image/jpg" type="file" name="pictures[]" id="pictures" multiple></label>
                    </p>
                </div>
                <div class="mb-3">
